...
Barnaby can be deployed to a Raspberry Pi on your home network, served behind nginx at `barnaby.joshpiserver.lan`.
...
4. Copy the nginx config and enable it:
   ```bash
   sudo cp ~/barnaby_ts/scripts/nginx/barnaby.joshpiserver.lan /etc/nginx/sites-available/
   sudo ln -s /etc/nginx/sites-available/barnaby.joshpiserver.lan /etc/nginx/sites-enabled/
...
- nginx reverse-proxies `barnaby.joshpiserver.lan` → `127.0.0.1:3001`
